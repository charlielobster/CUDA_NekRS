...
Go to the ParaView website to find a version of ParaView that can load nek5000 files without issue. 

[https://www.paraview.org/download/](https://www.paraview.org/download/)

I used the this link: [ParaView-6.0.1-MPI-Linux-Python3.12-x86_64.tar.gz](https://www.paraview.org/paraview-downloads/download.php?submit=Download&version=v6.0&type=binary&os=Linux&downloadFile=ParaView-6.0.1-MPI-Linux-Python3.12-x86_64.tar.gz)

I moved the extracted files into `/usr/local/Paraview`

```
cd ~/Downloads
tar -xvf ParaView-6.0.1-MPI-Linux-Python3.12-x86_64.tar.gz 
sudo mv ParaView-6.0.1-MPI-Linux-Python3.12-x86_64 /usr/local/ParaView
```

and added ParaView's `bin` folder to `PATH`:

```
echo export PATH=/usr/local/ParaView/bin:$PATH >> ~/.bashrc
```
...
[<img src="images/charl3_lawbstr_channel.png" />](https://www.youtube.com/playlist?list=PLkcISurbTBJsnNKwDAb1c73r7JVim71-H)
