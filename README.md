...
Parts of the NekRS target the compute-70 (CUDA Version 7) architecture. Hypre, one of NekRS's third party tools, is not considered compatible with Cuda Toolkit above version 11. There are references to some artifacts not found in the current version of the CUDA Toolkit. The NekRS build also uses flags which set the C++ standard to 11, and there are other flags, such as the OCCA environment flags, that are left empty, but could be configured to work in new ways during a future NekRS run.
