...
UPDATE 9/15 - This Readme is still in progress. While I am able to compile NekRS without issue now, I'm unable to run any NekRS samples without fatally crashing. The most recent error is a call to OpenMPI (MPI_WaitAll) inside 3rd_party/gslib/ogs/oogs.cpp, so I am trying to recompile ompi with better error handling, and suspect my OMPI build flags. It's also clear to me now that I shouldn't use the latest Nvidia drivers for this because one of the tools is assuming compute_70, which is no longer found in CUDA Toolkit 13, I might need to go back to 11.
